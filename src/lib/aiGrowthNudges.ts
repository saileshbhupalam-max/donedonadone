...
export function shouldShowReferralNudge(
  ...
  if (
    ...
    candidates.push({
      ...
      message: `${context.sessionCount} sessions! You are a DanaDone regular. Invite friends and earn credits.`,
      cta: 'Share your code',
      priority: 2,
    });
  }
...
export function generateNudgeMessage(
  ...
  switch (nudgeType) {
    case 'referral_post_session':
      return `Hey ${name}, that was a great session! Share DanaDone with a friend and earn ${config.credits.referralComplete} Focus Credits when they complete their first session.`;

    case 'referral_milestone':
      return `${context.sessionCount} sessions and counting! You clearly love DanaDone. Each friend you refer earns you ${config.credits.referralComplete} FC. Your code: ${context.referralCode || 'check your profile'}`;
    ...
    case 'referral_onboarding':
      return `Welcome to DanaDone, ${name}! Coworking is better with friends. Invite someone and you both get rewarded.`;
