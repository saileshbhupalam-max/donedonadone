...
describe('calendar', () => {
  describe('getGoogleCalendarUrl', () => {
    ...
    it('contains correct title with FocusClub prefix', () => {
      const url = getGoogleCalendarUrl({
        title: 'Deep Work Session',
        date: '2026-03-15',
      });
      const params = new URLSearchParams(url.split('?')[1]);
      expect(params.get('text')).toBe('FocusClub: Deep Work Session');
    });
    ...
    it('handles missing optional fields gracefully', () => {
      ...
      const params = new URLSearchParams(url.split('?')[1]);
      expect(params.get('text')).toBe('FocusClub: Session');
    ...
    it('includes session details in description', () => {
      ...
      expect(details).toContain('Check in with the FocusClub app');
    });
  });
});
